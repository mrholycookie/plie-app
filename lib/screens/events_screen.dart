...
class _EventsScreenState extends State<EventsScreen>
  ...
  void applyFilter() {
    ...
    visibleEvents.sort((Event a, Event b) {
      final aDate =
          a.startDate ?? DateTime.now().add(const Duration(days: 365));
      final bDate =
          b.startDate ?? DateTime.now().add(const Duration(days: 365));
                        ...
                        TextField(
                          ...
                          decoration: InputDecoration(
                            ...
                            focusedBorder: OutlineInputBorder(
                              borderRadius: BorderRadius.circular(8),
                              borderSide:
                                  const BorderSide(color: Color(0xFFCCFF00)),
                            ),
                          ),
                        ),
                        const SizedBox(height: 12),
  ...
  Future<void> _addToCalendar(Event event) async {
    ...
    try {
      ...
      try {
        await _addToCalendarViaIntent(event, event.startDate!,
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                  'Не удалось открыть календарь. Установите приложение календаря.'),
              backgroundColor: Colors.red,
  ...
  Future<void> _addToCalendarViaIntent(
  ...
  Widget _buildCalendar() {
    ...
    final eventsInMonth = _eventDates
        .where((date) =>
                    ...
                    return Row(
                      children: List.generate(7, (dayIndex) {
                        ...
                        final currentDate = DateTime(_selectedMonth.year,
                            _selectedMonth.month, dayNumber);
                        final normalizedDate = DateTime(currentDate.year,
                ...
                Row(
                  children: [
                    const Icon(Icons.calendar_today,
                        size: 12, color: Color(0xFFCCFF00)),
